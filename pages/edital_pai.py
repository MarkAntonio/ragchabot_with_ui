...
from chatbot_util.chatbot_util import RAG, llm_response
...
if 'messages' not in st.session_state:
    ...
    st.session_state.messages["edital_pai"] = []

for message in st.session_state.messages['edital_pai']:
...
if prompt:
    ...
    st.session_state.messages['edital_pai'].append({'role': 'user', 'content': prompt})
    
    session_id = 1 # apenas pra testr
    context_dict = {"disconnected_error": True}

    while context_dict["disconnected_error"]:
        context_dict = RAG(prompt, {"source": "Edital 01.2024 PAI"})
        if not context_dict["disconnected_error"]:
            ...
            st.session_state.messages['edital_pai'].append({'role': 'System', 'content': erro})
    ...
    st.session_state.messages['edital_pai'].append({
        'role': 'assistant',
        'content': {
            'text': answer['text'],
            'source': [
